...
with st.sidebar:
    ...
    cap_pct = st.slider("Cap (percentil do topo)", 80, 100, 95)
...
if missing:
    st.warning(f"{missing} ocorrência(s) com 'posto' não cadastrado em POSTOS (sem lat/long).")
...
# ----------------- Aggregate -----------------
agg_total = df_f.groupby(["lat", "long"]).size().reset_index(name="registros")
